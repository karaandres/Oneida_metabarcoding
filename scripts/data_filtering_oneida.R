...
ASV_count_by_sites <- subset(ASV_count_by_sites, select=-c(X,ASV)) # remove unneeded columns

### Remove ASVs with reads below threshold of 0.1% of all reads ###
...
for (i in 1:nrow(ASV_count_by_sites_cleaned)){ # for each ASV
  ...
  if (ASV_count_by_sites_cleaned$sscinames[i]=="NO MATCH"|ASV_count_by_sites_cleaned$sscinames[i]=="AMBIGUOUS"){
    ASV_count_by_sites_lowest_tax$sscinames[i] <- ASV_count_by_sites_cleaned$genus[i]
  ...
  if (ASV_count_by_sites_cleaned$genus[i]=="AMBIGUOUS"){
    ASV_count_by_sites_lowest_tax$sscinames[i] <- ASV_count_by_sites_cleaned$family[i]
...
sp_count_by_site <- rowsum(ASV_count_by_sites_lowest_tax[,1:29], group = ASV_count_by_sites_lowest_tax$scomnames, na.rm = TRUE)
